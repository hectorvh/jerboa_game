...
// Thin data-access layer (spec §4.4). In this frontend vertical slice it
// keeps records in memory and logs writes; swap the body of these functions
// for Supabase SQL (or any backend) without touching the UI.
...
export async function saveParticipant(
  ...
  if (existingId && store.participants.has(existingId)) {
    ...
    console.log('[v0] participant UPDATE', updated)
    return updated
  }
  ...
  console.log('[v0] participant INSERT', record)
  return record
}
...
export async function recordConsent(
  ...
  console.log('[v0] consent recorded', record)
  return record
}
